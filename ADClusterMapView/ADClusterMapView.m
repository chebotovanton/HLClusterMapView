...
- (void)_clusterInMapRect:(MKMapRect)rect
{
#warning calculate zoom level and pass to find method
	NSInteger level = 1100000/rect.size.height;
	NSLog(@"level: %i", level);
	NSLog(@"height: %.2f", rect.size.height);
	
	NSArray * clustersToShowOnMap = [_rootMapCluster findLevel:level childrenInMapRect:rect];
