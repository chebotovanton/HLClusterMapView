...
{
	CGFloat diffx = clusterOne.annotation.annotation.coordinate.latitude - clusterTwo.annotation.annotation.coordinate.latitude;
	diffx = fabsf(diffx);
...
- (NSArray *)findLevel:(NSInteger)level childrenInMapRect:(MKMapRect)mapRect
{
	...
	while(needsNextIteration){
		...
		for(ADMapCluster * cluster in clusters){
			BOOL canAddClustersChildren = YES;
